...
SortedList_t* lists;    // pointer to start of lists
pthread_mutex_t* mutex_locks;        // one lock for each sublist
int* spin_locks;

    // Hash by the ASCII value of the first character of key modulo the number of lists
...
void process_args(int argc, char * argv[])
{
    ...
    while (1)
    {
        option = getopt_long(argc, argv, "i:t:y:s:l:", options, NULL);
        if (option == -1) break;
        switch (option)
        {
          case 'i':
          ...
          case 'l':
              ...
              if (num_lists < 1)
              {
                  fprintf(stderr, "The number of sub-lists must be positive\n");
              }
              break;
          default:
                fprintf(stderr, "Correct options: --iterations= --threads= --lists= --sync= --yield=\n");
                exit(1);
        }
    }
}

void lock_func(int nl)
{
    if (sync_type == 'm' && pthread_mutex_lock(mutex_locks + nl) != 0)
    {
        fprintf(stderr, "Error locking mutex\n");
        exit(1);
    }
    if (sync_type == 's')
      while (__sync_lock_test_and_set(spin_locks + nl, 1))
        while (spin_locks[nl]) continue;
}

void unlock_func(int nl)
{
    if (sync_type == 's') __sync_lock_release(spin_locks + nl);
    else if (sync_type == 'm' && pthread_mutex_unlock(mutex_locks + nl) != 0)
    {
        fprintf(stderr, "Error locking mutex\n");
        exit(1);
    }
...
void* thread_worker(void* threadID)
{
    ...
    {
        list_num = hashkey((pool[thread_id] + i)->key);
        clock_gettime(CLOCK_MONOTONIC, &start_time);
        lock_func(list_num);
        clock_gettime(CLOCK_MONOTONIC, &end_time);
        total_wait_time += (1000000000 * (end_time.tv_sec - start_time.tv_sec) + end_time.tv_nsec - start_time.tv_nsec);
        SortedList_insert(lists + list_num, (pool[thread_id]) + i);
        unlock_func(list_num);
    }
    ...
    for (list_num = 0; list_num < num_lists; list_num++)
    {
        ...
        clock_gettime(CLOCK_MONOTONIC, &end_time);
        total_wait_time += (1000000000 * (end_time.tv_sec - start_time.tv_sec) + end_time.tv_nsec - start_time.tv_nsec);
        int cur_length = SortedList_length(lists + list_num);
        if (cur_length != 0)
        {
    ...
    for (int i = 0; i < num_iterations; i++)
    {
        ...
        clock_gettime(CLOCK_MONOTONIC, &end_time);
        total_wait_time += (1000000000 * (end_time.tv_sec - start_time.tv_sec) + end_time.tv_nsec - start_time.tv_nsec);
        SortedListElement_t* e = SortedList_lookup(lists + list_num, pool[thread_id][i].key);
...
void print_out(long long total_runtime)
{
    ...
    long long average_wait_time = total_wait_time / num_threads;
        // If no locks are used
    if (total_wait_time == 0)
    {
        average_wait_time = 0;
    }
    printf("%s,%d,%d,%d,%d,%lld,%lld,%lld\n", name, num_threads, num_iterations, num_lists, operations, total_runtime, avg_time_per_op, average_wait_time);
}
...
int main(int argc, char* argv[])
{
    ...
    signal(SIGSEGV, signalHandler);
        ...
        // Create head pointers of each linked list
    lists = (SortedList_t*) malloc(num_lists * sizeof(SortedList_t));
    if (lists == NULL)
    {
        fprintf(stderr, "Error with malloc for lists\n");
        exit(1);
    }
    for (int i = 0; i < num_lists; i++)
    {
        lists[i].next = lists[i].prev = &lists[i];
        lists[i].key = NULL;       // Head pointer has a key of NULL
    }
    ...
    for (int thread_num = 0; thread_num < num_threads; thread_num++)
    {
        ...
        if (pthread_create(&threads[thread_num], NULL, thread_worker, &thread_ids[thread_num]) != 0)
        {
